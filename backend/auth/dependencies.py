...
from services.user_service import get_or_create_user, ADMIN_EMAIL
...
async def get_current_user(
    ...
    # Extract user email from token payload or Clerk API
    email = payload.get("email") or payload.get("primary_email_address")
    if not email:
        try:
            clerk_user = clerk_sdk.users.get(user_id=clerk_id)
            if clerk_user and getattr(clerk_user, "email_addresses", None):
            ...
            print("[get_current_user] Could not fetch email from Clerk API:", err)
...
async def check_and_consume_ai_quota(
    ...
    """Enforces 5 AI requests limit for free users; aradwadtushar72@gmail.com and admins get unlimited requests."""
    is_admin = (
        user.role == "admin"
    ...
    if user.ai_requests_count >= FREE_AI_LIMIT:
        raise HTTPException(
            ...
            detail=f"You have reached your free limit of {FREE_AI_LIMIT} AI requests. Please contact administrator ({ADMIN_EMAIL}) for unlimited access.",
        )
