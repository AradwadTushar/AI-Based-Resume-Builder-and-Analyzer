  ...
  return (
    <div className="w-full bg-white text-[#111827] antialiased select-text max-w-[210mm] mx-auto p-[18mm] shadow-md border border-gray-100 print:max-w-none print:shadow-none print:border-none print:p-0">
      ...
      <header className="text-center pb-[10pt]">
        ...
        {personalInfo?.title && (
          <p className="font-sans text-[11pt] font-normal text-[#1a56a0] mt-[3pt] tracking-[0.06em] uppercase">
            {personalInfo.title}
          </p>
        )}
        <div className="mt-[6pt] text-[9pt] text-[#374151] font-sans leading-[1.6]">
          {[
            ...
            <React.Fragment key={idx}>
              <span>{item}</span>
              {idx < arr.length - 1 && <span className="mx-[5pt] text-[#1a56a0] font-bold select-none">•</span>}
            </React.Fragment>
          ))}
        </div>
      </header>

      {/* ── TOP NAVY DIVIDER BAR ── */}
      <div className="h-[2.5pt] bg-[#1a56a0] mt-[8pt] mb-0" />

      {/* ── SUMMARY ── */}
      {summary && (
        <section className="mt-[12pt] break-inside-avoid print:break-inside-avoid">
          <h2 className="font-sans text-[10pt] font-bold text-[#1a56a0] uppercase tracking-[0.10em] pb-[3pt] border-b border-[#d1d5db] mb-[7pt]">
              ...
              {job.bullets && job.bullets.length > 0 ? (
                <ul className="list-none p-0 mt-[4pt]">
                  {job.bullets.map((bullet, bIdx) => (
                    <li key={bIdx} className="relative pl-[10pt] text-[9.5pt] text-[#374151] leading-[1.5] mb-[2.5pt] before:content-['–'] before:absolute before:left-0 before:text-[#1a56a0] before:font-bold">
                      {bullet}
                    </li>
                  ))}
                </ul>
                ...
                <div className="float-left max-w-[72%]">
                  <span className="block text-[9.5pt] font-bold text-[#111827] leading-[1.35]">
                    ...
                    {(project.url || project.githubLink) && (
                      <span className="font-normal text-[#1a56a0] text-[8.5pt]"> — {project.url || project.githubLink}</span>
                    )}
                  </span>
                  {project.techStack && (
